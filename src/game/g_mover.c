...
void Touch_Button( gentity_t *ent, gentity_t *other, trace_t *trace )
{
  ...
  if( !other->client )
    return;
	/*if( !strcmp(other->password, "desde") && !strcmp(ent->target,"nullpassword"))
	{
		parent = &g_entities[ 0 ];
		for( i = 0; i < level.num_entities; i++, parent++ )
		{
			if(!strcmp(parent->targetname, "server_password_door0_trigger"))
			{
				if( parent->moverState == MOVER_POS1 )
				{
					Use_BinaryMover( parent, ent, other );
					trap_SendServerCommand( -1, "print \"Door opened!\n\"" );
				}
			}
		}
		trap_SendServerCommand( -1, "cp \"Secret door opened!\"" );
	}*/
  if( ent->moverState == MOVER_POS1 )
