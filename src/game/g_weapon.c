...
void cancelBuildFire( gentity_t *ent )
{
  ...
  if( ent->client->ps.stats[ STAT_PTEAM ] == PTE_HUMANS )
  {
        ...
        /*traceEnt->spawned &&*/ traceEnt->health > 0 )
    {
      ...
      //traceEnt->health += HBUILD_HEALRATE + (g_ambush_upgrade.integer * 5);
      ...
      {
        // G_AddCreditToClient( ent->client, HBUILD_HEALRATE + (g_ambush_upgrade.integer * 5), qtrue );
...
void buildFire( gentity_t *ent, dynMenu_t menu )
{
  ...
  {
    if( ent->client->ps.stats[ STAT_MISC ] > 0 && g_buildTimer.integer > 0)
    {
      G_AddEvent( ent, EV_BUILD_DELAY, ent->client->ps.clientNum );
      return;
    }
    ...
    if( G_BuildIfValid( ent, ent->client->ps.stats[ STAT_BUILDABLE ] & ~SB_VALID_TOGGLEBIT ) )
    {
      ...
      else if( ent->client->ps.stats[ STAT_PTEAM ] == PTE_ALIENS && !G_IsOvermindBuilt( ) && g_buildTimer.integer > 0)
      {
        ent->client->ps.stats[ STAT_MISC ] +=
          BG_FindBuildDelayForWeapon( ent->s.weapon ) * 2;
      }
      else if( ent->client->ps.stats[ STAT_PTEAM ] == PTE_HUMANS && !G_IsPowered( muzzle ) &&
          ( ent->client->ps.stats[ STAT_BUILDABLE ] & ~SB_VALID_TOGGLEBIT ) != BA_H_REPEATER && g_buildTimer.integer > 0) //hack
      {
        ent->client->ps.stats[ STAT_MISC ] +=
          BG_FindBuildDelayForWeapon( ent->s.weapon ) * 2;
      }
      else
	if(g_buildTimer.integer > 0)
	{
        ent->client->ps.stats[ STAT_MISC ] +=
...
void FireWeapon( gentity_t *ent )
{
  ...
  switch( ent->s.weapon )
  {
    ...
    case WP_BLASTER:
      blasterFire( ent );
      //bulletFire( ent, BLASTER_SPREAD, BLASTER_DMG, MOD_BLASTER );
      break;
    case WP_MACHINEGUN:
