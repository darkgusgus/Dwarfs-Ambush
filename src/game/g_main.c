...
#define QVM_NAME       "Dwarfs~{Official!}"
...
static cvarTable_t   gameCvarTable[ ] =
{
  ...
  { &g_logFile, "g_logFile", "logs.log", CVAR_ARCHIVE, 0, qfalse  },
  ...
  { &g_ambush_upgrade_cost, "g_ambush_upgrade_cost", "30000", CVAR_ARCHIVE, 0, qfalse  },
...
void CheckExitRules( void )
{
        ...
        ( level.numLiveAlienClients == 0 ) ) )
  {
    ...
    if (g_ambush.integer == 1)
    {
      ...
      if (g_ambush_stage_suicide.integer == 1)
        trap_SendServerCommand( -1, va("print \"Humans win (^3Time record: %s^7) (Suicide: ^2On^7)\n\"", string));
      else
        trap_SendServerCommand( -1, va("print \"Humans win (^3Time record: %s^7) (Suicide: ^9Off^7)\n\"", string));
    }
    else
             ...
             ( level.numLiveHumanClients == 0 ) ) )
  {
    ...
    trap_SendServerCommand( -1, "print \"Aliens win (^3Time record: %s^7)\n\"");
