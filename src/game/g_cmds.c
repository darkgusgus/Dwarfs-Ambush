...
void G_ChangeTeam( gentity_t *ent, pTeam_t newTeam )
{
  ...
  //make sure it doesnt let them save funds all the time
...
void Cmd_Team_f( gentity_t *ent )
{
  ...
  // stop team join spam
  if( level.time - ent->client->pers.teamChangeTime < 3000 )
    return;
...
void Cmd_CallVote_f( gentity_t *ent )
{
    ...
    !Q_stricmp( arg1, "unmute" ) )
  {
    ...
    if(strchr(reason,';'))
    {
	trap_SendServerCommand( ent-g_entities, "print \"^1Invalid vote string. IP logged.\n\"" );
        return;
    }
  ...
  //trap_SendServerCommand( -1, "cp \"VOTE\n\"" );
...
void Cmd_Destroy_f( gentity_t *ent )
{
  ...
  if( ent->client->ps.stats[ STAT_STATE ] & SS_HOVELING )
  {
    if( ( ent->client->hovel->s.eFlags & EF_DBUILDER ) &&
      !ent->client->pers.designatedBuilder )
    {
      // Flood limit if they keep trying to decon a protected structure
      if( g_floodMinTime.integer && G_Flood_Limited( ent ) )
        trap_SendServerCommand( ent-g_entities,
          "print \"You are now flood limited.\n\"" );
      else
      {
        trap_SendServerCommand( ent-g_entities, 
  ...
  if( !( ent->client->ps.stats[ STAT_STATE ] & SS_INFESTING ) )
  {
          ...
          ( ent->client->ps.weapon <= WP_HBUILD ) ) )
    {
      ...
      if( g_deconMode.integer == 2 && traceEnt->deconstruct )
      {
      ...
      if(!ent->client->pers.designatedBuilder &&
        (traceEnt->s.modelindex == BA_H_REACTOR || traceEnt->s.modelindex == BA_A_OVERMIND) )
      {
        trap_SendServerCommand( ent-g_entities, 
            va( "print \"Only a designated player can move the %s\nUse /callteamvote designate [name]\n\"",
                (traceEnt->s.modelindex == BA_H_REACTOR) ? "Reactor" : "Overmind" ) );
        return;
      }
 
      // Prevent destruction of the last spawn
      if( g_deconMode.integer != 2 && !g_cheats.integer )
      {
        if( ent->client->pers.teamSelection == PTE_ALIENS &&
      ...
      if( !g_buildMode.integer || !g_deconMode.integer )
      {
        trap_SendServerCommand( ent-g_entities,
          "print \"Deconning is not allowed\n\"" );
        return;
      }
...
void Cmd_Build_f( gentity_t *ent )
{
  ...
  if( g_buildMode.integer == 0 )
  {
    trap_SendServerCommand( ent-g_entities,
      "print \"Building has been turned off.\n\"" );
    return;
  }
...
void Cmd_Node_f( gentity_t *ent )
{
	...
	if(!ent->patheditor || !(G_admin_permission( ent, "drawnodes" )) )
	{
		trap_SendServerCommand( ent-g_entities,
			"print \"You do not have permission to edit the path nodes.\n\"");
		return;
	}
